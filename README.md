...
> 注：在线演示版会把上传的文本发送到云端服务器处理；若在意隐私，请使用本地版（见下方「基本使用步骤」）。
...
- **隐私安全**：本地版（`start.bat`）会把文件发送给本机 Flask 服务处理，应用代码不会主动保存上传原文；在线演示版会将上传文本发送到远程服务器处理。当前在线演示使用 HTTP，不应视为加密传输，请勿上传敏感、私密或未获授权的文本。
